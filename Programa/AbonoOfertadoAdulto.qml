    ...
    Connections {
        ...
        onSig_listaAbonosAdultosEnOferta: {
            ...
            for (x=0;x<lista.length;x++){
                if (Number(strNumero) === lista[x].total_clases) {
    ...
    function actualizar() {
        ...
        if (available) {
            ...
            if (idAbonoOfertado === -1)
                idAbonoOfertado = wrapper.managerAbono.altaDeAbonoAdulto(spinPrecio.value,strNumero)
            else
                wrapper.managerAbono.actualizarAbonoOfertado(idAbonoOfertado,spinPrecio.value,switchButton.checked)
        }
