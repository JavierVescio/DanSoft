...
Rectangle {
    ...
    property int idClaseActividad : 0
    ...
    Connections {
        ...
        onSig_listaClaseAsistencias: {
            //Qt.formatDateTime(styleData.value,"dd/MM/yyyy ddd HH:mm")
    ...
    function obtenerPresentes() {
        ...
        if (rangoDias > 0 && idClaseActividad > 0) {
            ...
            if (!wrapper.managerAsistencias.obtenerAsistenciasEntreFechasPorActividad(idClaseActividad, txtFechaInicial.date, txtFechaFinal.date)) {
                //Mostrar mensajito
            }

            if (!wrapper.managerAsistencias.obtenerAsistenciasInfantilesEntreFechasPorActividad(idClaseActividad, txtFechaInicial.date, txtFechaFinal.date)) {
    ...
    SelectorClaseDeDanza {
        ...
        z: 10
        visible: false
        verSoloActividad: true

        onActividadAsistencia: {
            noHayAsistencias()
            lblAsistiendoActividad.text = "\t" + nombreActividad
            idClaseActividad = idActividad
            obtenerPresentes()
        }

        onClaseNoSeleccionada: {
            lblAsistiendoActividad.text = strSinInformacion
            idClaseActividad = -1
            noHayAsistencias()
        }
    }
    ...
    Column {
        ...
        Row {
            ...
            Button {
                text: qsTrId("Seleccionar actividad")
